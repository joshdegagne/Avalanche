...
void ConeTexturedModel::InitializeModel(float height, float radius, int nFaces, WCHAR** pTextureFileNames)
{
	...
	/*
	pTextureFileNames is expected to be an array of 2 items
	pTextureFileName[0] is the texture applied to the top end of the cone
	pTextureFileName[1] is the texture applied to the bottom end of the cone
	*/

	int numberOfFaces = nFaces;

	m_textureFileNames = new WCHAR*[NUMBER_OF_TEXTURES]; //file names of 2 face .dds texture files
    for(int i=0; i<NUMBER_OF_TEXTURES; i++){
		m_textureFileNames[i] = pTextureFileNames[i]; //record the file names of the 2 cone face texture files
	}
	...
	//changing the sign of angle will affect whether the inside or outside of the cone
	//is visible
	float angle = -XM_PI * 2 / nFaces; //slice angle of each face



	//temporary vertices for bottom
    ...
    //define the vertices around the top and bottom of cone
    ...
    for (int i = 0; i < numberOfFaces; i++)
    {
		   ...
		   //top slice triangle
		   m_topTextureVertices[3*i+0].position = bottomVertices[i];  
	       m_topTextureVertices[3*i+0].texture = XMFLOAT2((radius + bottomVertices[i].x)/(2.0f * radius) , (radius - bottomVertices[i].z)/(2.0f * radius));
	       m_topTextureVertices[3*i+1].position = topCenter;   //center
	       m_topTextureVertices[3*i+1].texture = XMFLOAT2(0.5f , 0.5f);
	       m_topTextureVertices[3*i+2].position = bottomVertices[i + 1];   
	       m_topTextureVertices[3*i+2].texture = XMFLOAT2((radius + bottomVertices[i+1].x)/(2.0f * radius), (radius - bottomVertices[i+1].z)/(2.0f * radius));
...
bool ConeTexturedModel::initializeTextures(ID3D11Device* d3dDevice){
	...
	const int NUMBER_OF_TEXTURES = 2; //one for the sides and one each for top and bottom
...
bool ConeTexturedModel::Render(ID3D11DeviceContext* deviceContext,  XMFLOAT4X4 viewMatrix, XMFLOAT4X4 projectionMatrix, ColorShader* colorShader, TextureShader* textureShader){
